...
class SensorManager:
    ...
    def create_new_sensor(self, changes: list):
        ...
        self.sensors_per_type[
            changes[1]][changes[0]] = self.sensors[changes[0]]

    def update_sensor(self, name: str, changes: list):
        self.sensors[name].update(changes)

    def get_sensor_details(self, name: str):
        returnal = [self.sensors[name].name(), self.sensors[name].type(
        ), self.sensors[name].conexion(), self.sensors[name].commands()]
        return returnal
